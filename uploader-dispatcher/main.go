...
import (
	...
	"scutbot.cn/web/rm-monitor/uploader-dispatcher/internal/logic"
...
func init() {
	logx.MustSetup(logx.LogConf{ServiceName: "uploader-dispatcher", Mode: "console", Encoding: "plain"})
}
...
func main() {
	...
	for {
		...
		if err := logic.NewDispatchLogic(ctx, svcCtx).Tick(); err != nil {
			logx.Errorf("uploader dispatch tick failed: %v", err)
		}
		cancel()
		select {
		case <-wake:
...
func listen(dsn string, wake chan<- struct{}) {
	for {
		...
		if err != nil {
			logx.Errorf("uploader dispatcher listener failed: %v", err)
