...
import (
	...
	"scutbot.cn/web/rm-monitor/ocr-dispatcher/internal/logic"
...
func init() {
	logx.MustSetup(logx.LogConf{ServiceName: "ocr-dispatcher", Mode: "console", Encoding: "plain"})
}
...
func main() {
	...
	for {
		...
		if err := logic.NewDispatchLogic(ctx, svcCtx).Tick(); err != nil {
			logx.Errorf("ocr dispatch tick failed: %v", err)
		}
		cancel()
		select {
		case <-wake:
...
func listen(dsn string, wake chan<- struct{}) {
	for {
		...
		if err != nil {
			logx.Errorf("ocr dispatcher listener failed: %v", err)
