...
func migrateLegacyLarkMessages(ctx context.Context, db *stdsql.DB) error {
	...
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS lark_card_messages (
			...
			lark_message_card_messages integer NOT NULL
		)`,
		`WITH keepers AS (
		...
		INSERT INTO lark_card_messages (message_id, created_at, updated_at, lark_message_card_messages)
		SELECT lm.message_id, lm.created_at, lm.updated_at, k.keep_id
		FROM lark_messages lm
		JOIN keepers k ON k.match_lark_messages = lm.match_lark_messages
		WHERE lm.message_id IS NOT NULL AND lm.message_id <> ''
