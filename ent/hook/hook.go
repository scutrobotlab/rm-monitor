...
// Mutate calls f(ctx, m).
func (f LarkMessageFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.LarkMessageMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.LarkMessageMutation", m)
}
...
// The MatchRoundFunc type is an adapter to allow the use of ordinary
// function as MatchRound mutator.
type MatchRoundFunc func(context.Context, *ent.MatchRoundMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MatchRoundFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MatchRoundMutation); ok {
		return f(ctx, mv)
	}
