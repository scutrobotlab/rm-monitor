...
type TeamQuery struct {
	...
	withRedMatches  *MatchQuery
	withBlueMatches *MatchQuery
...
// QueryRedMatches chains the current query on the "red_matches" edge.
func (_q *TeamQuery) QueryRedMatches() *MatchQuery {
...
// QueryBlueMatches chains the current query on the "blue_matches" edge.
func (_q *TeamQuery) QueryBlueMatches() *MatchQuery {
	query := (&MatchClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *TeamQuery) Clone() *TeamQuery {
	...
	return &TeamQuery{
		...
		withRedMatches:  _q.withRedMatches.Clone(),
		withBlueMatches: _q.withBlueMatches.Clone(),
...
// WithRedMatches tells the query-builder to eager-load the nodes that are connected to
// the "red_matches" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TeamQuery) WithRedMatches(opts ...func(*MatchQuery)) *TeamQuery {
	query := (&MatchClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRedMatches = query
	return _q
}

// WithBlueMatches tells the query-builder to eager-load the nodes that are connected to
// the "blue_matches" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TeamQuery) WithBlueMatches(opts ...func(*MatchQuery)) *TeamQuery {
	query := (&MatchClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withBlueMatches = query
	return _q
}
...
func (_q *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.withRedMatches != nil,
			_q.withBlueMatches != nil,
		}
	)
	...
	if query := _q.withRedMatches; query != nil {
		if err := _q.loadRedMatches(ctx, query, nodes,
			func(n *Team) { n.Edges.RedMatches = []*Match{} },
			func(n *Team, e *Match) { n.Edges.RedMatches = append(n.Edges.RedMatches, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withBlueMatches; query != nil {
		if err := _q.loadBlueMatches(ctx, query, nodes,
			func(n *Team) { n.Edges.BlueMatches = []*Match{} },
			func(n *Team, e *Match) { n.Edges.BlueMatches = append(n.Edges.BlueMatches, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TeamQuery) loadRedMatches(ctx context.Context, query *MatchQuery, nodes []*Team, init func(*Team), assign func(*Team, *Match)) error {
	...
	query.Where(predicate.Match(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.RedMatchesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.team_red_matches
		if fk == nil {
			return fmt.Errorf(`foreign-key "team_red_matches" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "team_red_matches" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *TeamQuery) loadBlueMatches(ctx context.Context, query *MatchQuery, nodes []*Team, init func(*Team), assign func(*Team, *Match)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Match(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.BlueMatchesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.team_blue_matches
		if fk == nil {
			return fmt.Errorf(`foreign-key "team_blue_matches" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "team_blue_matches" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
